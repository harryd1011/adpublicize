...
if (isset($_POST["update"])) {
    ...
    if ($slot_row) {
        $update = "UPDATE slot_details_1 SET Page_No='$Page_No', Position='$Position', Size='$Size', Price='$Price', Status='$Status', Publish_Date='$Publish_Date' where Page_No='$Page_No' and Position='$Position' ";

        if (mysqli_query($conn, $update)) {
            echo "<script>alert('Updated Successfully')</script>";
        } else {
            echo "<script>alert('We ran into a problem please try again later.')</script>";
        }
    } else {
        echo "<script>alert('No such slot is alloted.')</script>";
    }
}
?>
<!DOCTYPE html>
        ...
        <div id="page-content-wrapper" style="background-image: url('photos/scott-webb-sk59I1qRfEM-unsplash.jpg'); background-size:cover;">
            <nav class="navbar navbar-expand-lg py-4 px-4">
                <div class="d-flex align-items-center">
                    ...
                    <h2 class="fs-2 m-0 bg-light">Slot Update Form</h2>
                </div>
